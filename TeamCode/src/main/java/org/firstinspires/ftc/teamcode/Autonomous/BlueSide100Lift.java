...
@Autonomous(name = "BlueSide90Lift", group = "Autonomous")
public class BlueSide100Lift extends AutoOpMode {
    public BlueSide100Lift() {
        super();
    }
...
//        bringDownShooter(-.4, 620);
        ...
        if(getGyroYaw() + 40 > 360) {
            ...
            turnRightWithPID(firstTurn, .0055, .00003, 0.0);
            turnRightWithPID(40 - firstTurn, .006, .00003, 0.0);
        }
        else
            turnRightWithPID(40, .0045, .00003, 0.0);
        angle45 += 40;
        moveForwardPID(3100, angle45);
        sleep(750);
        double turn = 150;
        if(getGyroYaw() + turn > 360) {
            double firstTurn = Math.abs(360 - getGyroYaw());
            turnRightWithPID(firstTurn, .006, .000025, 0.0);
            turnRightWithPID(turn - firstTurn, .005, .000025, 0.0);
        }
        else
            turnRightWithPID(turn, .005, .000045, 0.0);
        resetEncoders();
...
//        moveForwardsToWhiteLine(300, perpendicular);
        moveToSecondLine(4000, -.3, startLight + 3);
        sleep(500);
        if(onWhiteLine() == false)
