...
//            BL.setPower(-output);
    ...
    public void moveForwardPIDtoWallRed(int distance, double angle, double time) throws InterruptedException {
        //.00025, .00000003, 0.0, 4000
        //calibration constants
        double p = .000275; double i = .000000275; //double d = 2.0;
        double error = distance;
        ...
        boolean stuck = false;
        while (Math.abs(getAvg() - beforeALV) < distance && !stuck) {
            error = distance - Math.abs(getAvg() - beforeALV);
            //proportional
            proportional = error * p;
            //integral
...
//        }
    ...
    public void moveToWallBlue(int distance, double power, double light) throws InterruptedException {
        ...
        while (Math.abs(getAvg() - beforeALV) < distance && Math.abs(colorSensorAverageValues(colorSensorWLA) - whiteACV) > 8) {
            ...
            if (System.currentTimeMillis() - distTime > time) {
                if(Math.abs(getAvg() - distALV) < 10) {
                    moveForwardWithEncoders(.25, 150);
                    turnRightWithGyroOneSide(-.25, 10);
                }
                else {
                    distTime = System.currentTimeMillis();
